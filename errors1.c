...
/**
 * print_shell_error - prints an error message
 * @info: the parameter & return info struct
 * @estr: string containing specified error type
 * Return: 0 if no numbers in string, converted number otherwise
 *        -1 on error
 */
void print_shell_error(info_t *info, char *estr)
{
	_print_to_stderr(info->fname);
	_print_to_stderr(": ");
	print_integer(info->line_count, STDERR_FILENO);
	_print_to_stderr(": ");
	_print_to_stderr(info->argv[0]);
	_print_to_stderr(": ");
	_print_to_stderr(estr);
}
...
/**
 * int_to_str - converter function, a clone of itoa
 * @num: number
 * @base: base

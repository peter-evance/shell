 ...
 * Return: bytes read
 */
ssize_t buffer_chained_commands(shell_info *info, char **buf, size_t *len)
{
    ssize_t r = 0;
    size_t len_p = 0;
...
ssize_t get_input(shell_info *info)
{
    ...
    if (len) /* we have commands left in the chain buffer */
    {
        ...
        check_chain_condition(info, buf, &j, i, len);

        while (j < len) /* iterate to semicolon or end */
        {
            if (is_chain_separator_separator(info, buf, &j))
                break;
            j++;
        }
 ...
 * Return: s
 */
int get_line(shell_info *info, char **ptr, size_t *length)
{
